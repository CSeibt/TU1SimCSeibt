...
void GetPositionHistogram(TTree* tree, string name) {
    ...
    tree->Draw(("posY:posX>>" + name + "_h2_x_y").c_str(), "", "goff");
    tree->Draw(("posZ:posX>>" + name + "_h2_x_z").c_str(), "", "goff");
    tree->Draw(("posZ:posY>>" + name + "_h2_y_z").c_str(), "", "goff");
    tree->Draw(("posZ:posY:posX>>" + name + "_h3_xyz").c_str(), "", "goff");
    ...
    h2_x_y->SetTitle("x-y");
    h2_x_y->GetXaxis()->SetTitle("x [cm]");
    h2_x_y->GetYaxis()->SetTitle("y [cm]");
    h2_x_y->Draw("colz");
    c1->cd(2);
    gPad->SetRightMargin(0.15);
    h2_x_z->GetZaxis()->SetTitleOffset(1.4);
    h2_x_z->SetTitle("x-z");
    h2_x_z->GetXaxis()->SetTitle("x [cm]");
    h2_x_z->GetYaxis()->SetTitle("z [cm]");
    h2_x_z->Draw("colz");
    c1->cd(3);
    gPad->SetRightMargin(0.15);
    h2_y_z->GetZaxis()->SetTitleOffset(1.4);
    h2_y_z->SetTitle("y-z");
    h2_y_z->GetXaxis()->SetTitle("y [cm]");
    h2_y_z->GetYaxis()->SetTitle("z [cm]");
    h2_y_z->Draw("colz");
    c1->cd(4);
    gPad->SetRightMargin(0.15);
    h3_xyz->GetZaxis()->SetTitleOffset(1.4);
    h3_xyz->SetTitle("x-y-z");
    h3_xyz->GetXaxis()->SetTitle("x [cm]");
    h3_xyz->GetYaxis()->SetTitle("y [cm]");
    h3_xyz->GetZaxis()->SetTitle("z [cm]");
    h3_xyz->Draw("box");
    c1->SaveAs((name + "_position.png").c_str());
}

void peakanalyse_simulation_40K()
{
    
    const int num_spek = 1.;
    const int num_files = 1;
    TString Rootfiles[num_files] = {"run40K.root"};//{"run_122keV_25mm.root"};
    
    const int num_peaks = 1;
    
    double MP[num_peaks] = {1460.820/1000.}; 
    int fix[num_peaks];
...
// Eventpositionen plotten

    TTree* tree = (TTree*)fTest[0]->Get("ions");
    GetPositionHistogram(tree, "ions");
...
//==========================================GESAMTES SPEKTRUM============================================================
  ...
  /*  for(int i=0;i<histo_main[0]->GetNbinsX();i++){
   ...
   if(Gauss->GetParameter(2)<0.3/1000.)
    ...
    long long num_events_sim = 0;
    for (int i = 0; i < num_files; i++) {
        TTree* tree = (TTree*)fTest[i]->Get("ions");
        if (tree) {
            num_events_sim += tree->GetEntries();
        }
    }
    ...
    cout << "Total number of simulated events: " << num_events_sim << endl;
    double eff_line = Peakcontent[j] / (num_events_sim); // 0.000117 is the branching ratio for 40K to decay via beta- decay
    double eff_line_uncert = uncert[j] / (num_events_sim);
    cout << "Detection efficiency for " << MP[j]*1000 << " keV line: " << eff_line << " +- " << eff_line_uncert << endl;
